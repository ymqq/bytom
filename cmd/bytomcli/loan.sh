...
duetime="2017-01-02*15:05:05"

echo "*** create control program for jack"
viewcmd="$tools create-control-program $jack"
echo $viewcmd
jack_program=`eval $viewcmd | sed -n '3p' | cut -d ":" -f 3 | cut -d "]" -f 1` 
echo -e "control program:$jack_program\n"

echo "*** create control program for alice"
viewcmd="$tools create-control-program $alice"
echo $viewcmd
alice_program=`eval $viewcmd | sed -n '3p' | cut -d ":" -f 3 | cut -d "]" -f 1` 
echo -e "control program:$alice_program\n"

#genenate the bytecode for the contract of LoanCollateral
echo "*** genenate the bytecode for the contract of LoanCollateral"
ivy="$GOPATH/src/github.com/bytom/exp/ivy/ivy"
viewcmd="$ivy LoanCollateral $gold 77 $duetime $jack_program $alice_program"
echo $viewcmd
...
echo $ouputid
viewcmd="./tools unlockLoanCollateral $ouputid $alice_program $gold 123456 88 abc 00000000 $gold 77 $bob  $jack_program"
echo $viewcmd
