...
echo "*** create control program for bob"
viewcmd="$tools create-control-program $bob"
echo $viewcmd
bobprogram=`eval $viewcmd | sed -n '3p' | cut -d ":" -f 3 | cut -d "]" -f 1` 
echo -e "control program:$bobprogram\n"

echo "*** create control program for jack"
viewcmd="$tools create-control-program $jack"
echo $viewcmd
jackprogram=`eval $viewcmd | sed -n '3p' | cut -d ":" -f 3 | cut -d "]" -f 1` 
echo -e "control program:$jackprogram\n"

#genenate the bytecode for the contract of Escrow
echo "*** genenate the bytecode for the contract of Escrow"
ivy="$GOPATH/src/github.com/bytom/exp/ivy/ivy"
viewcmd="$ivy Escrow $pubkey $bobprogram $jackprogram"
echo $viewcmd
...
echo $ouputid
viewcmd="$tools unlockEscrow $ouputid $jackprogram $gold 123456 88 abc 00000000 $alicerootpub $path1 $path2 $alice"
echo $viewcmd
result=`eval $viewcmd`
echo "$result"
