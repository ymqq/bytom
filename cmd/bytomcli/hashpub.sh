...
#genenate the sha3 hash for pubkey
echo "*** genenate the sha3 hash for pubkey"
sha3hash="$GOPATH/src/github.com/bytom/exp/ivy/tools/tools"
viewcmd="$sha3hash $pubkey"
echo $viewcmd
pubhash=`eval $viewcmd | sed -n '2p' | cut -d ":" -f 2`
echo -e "the hash of pubkey:$pubhash\n" 

#genenate the bytecode for the contract of LockWithPublicKeyHash
echo "*** genenate the bytecode for the contract of LockWithPublicKeyHash"
ivy="$GOPATH/src/github.com/bytom/exp/ivy/ivy"
viewcmd="$ivy LockWithPublicKeyHash $pubhash"
echo $viewcmd
...
echo $ouputid
viewcmd="$tools unlockPubKeyHash $ouputid $alice $gold 123456 88 abc $pubkey $alicerootpub $path1 $path2"
echo $viewcmd
result=`eval $viewcmd`
echo "$result"
