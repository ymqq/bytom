...
#genenate the bytecode for the contract of LockWithPublicKey
echo "*** genenate the bytecode for the contract of LockWithPublicKey"
ivy="$GOPATH/src/github.com/bytom/exp/ivy/ivy"
viewcmd="$ivy LockWithPublicKey $pubkey"
echo $viewcmd
...
echo $ouputid
viewcmd="$tools unlockPubKey $ouputid $alice $gold 123456 88 abc $alicerootpub $path1 $path2"
echo $viewcmd
result=`eval $viewcmd`
echo "$result"
