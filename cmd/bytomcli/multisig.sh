...
pubkey1=`cat $file | sed -n '3p' | cut -d ":" -f 2`
path11=`cat $file | sed -n '5p' | cut -d ":" -f 2`
path12=`cat $file | sed -n '6p' | cut -d ":" -f 2`
idx1=`cat $file | sed -n '8p' | cut -d ":" -f 2`
echo "pubkey:$pubkey1"
echo "path11:$path11"
echo "path12:$path12"
echo -e "idx1:$idx1\n"

echo "*** create pubkey for bob"
viewcmd="$tools pubkey $bob"
echo $viewcmd

result=`eval $viewcmd > $file`
pubkey2=`cat $file | sed -n '3p' | cut -d ":" -f 2`
path21=`cat $file | sed -n '5p' | cut -d ":" -f 2`
path22=`cat $file | sed -n '6p' | cut -d ":" -f 2`
idx2=`cat $file | sed -n '8p' | cut -d ":" -f 2`
echo "pubkey2:$pubkey2"
echo "path21:$path21"
echo "path22:$path22"
echo -e "idx2:$idx2\n"

echo "*** create pubkey for jack"
viewcmd="$tools pubkey $jack"
echo $viewcmd

result=`eval $viewcmd > $file`
pubkey3=`cat $file | sed -n '3p' | cut -d ":" -f 2`
path31=`cat $file | sed -n '5p' | cut -d ":" -f 2`
path32=`cat $file | sed -n '6p' | cut -d ":" -f 2`
idx3=`cat $file | sed -n '8p' | cut -d ":" -f 2`
echo "pubkey3:$pubkey3"
echo "path31:$path31"
echo "path32:$path32"
echo -e "idx3:$idx3\n"

#genenate the bytecode for the contract of LockWithMultiSig
echo "*** genenate the bytecode for the contract of LockWithMultiSig"
ivy="$GOPATH/src/github.com/bytom/exp/ivy/ivy"
viewcmd="$ivy LockWithMultiSig $pubkey1 $pubkey2 $pubkey3"
echo $viewcmd
...
echo $ouputid
viewcmd="$tools unlockMultiSig $ouputid $alice $gold 123456 88 abc $alicerootpub $path11 $path12 $bobrootpub $path21 $path22"
echo $viewcmd
result=`eval $viewcmd`
echo "$result"
