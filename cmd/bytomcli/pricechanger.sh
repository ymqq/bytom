...
echo "*** create asset silver"
silverrootpub=`./bytomcli create-key silver 123456 | sed -n '2p' | cut -d ":" -f 2`
echo "silver root pubkey:" $silverrootpub

silver=`./bytomcli create-asset silver $silverrootpub | cut -d ":" -f 2`
echo -e "silver asset_id:$silver \n" 
...
echo "*** issue asset 20000 silver to alice"
viewcmd="./bytomcli sub-create-issue-tx $alice $silver 20000 123456"
echo $viewcmd
issuecmd="$viewcmd | sed -n '4p' | cut -d "[" -f 3 | cut -d "]" -f 1"
result=`eval $issuecmd`
echo "$result"
...
#genenate the bytecode for the contract of PriceChanger
echo "*** genenate the bytecode for the contract of PriceChanger"
ivy="$GOPATH/src/github.com/bytom/exp/ivy/ivy"
viewcmd="$ivy PriceChanger 77 $gold $pubkey $control_program"
echo $viewcmd
...
#genenate the bytecode for the contract of PriceChanger(inner PriceChanger)
echo "*** genenate the bytecode for the contract of PriceChanger(inner PriceChanger)"
innerivy="$GOPATH/src/github.com/bytom/exp/ivy/ivy"
viewcmd="$innerivy PriceChanger 99 $silver $pubkey $control_program"
echo $viewcmd
innercode=`eval $viewcmd | sed -n '2p'`
echo -e "contract bytecode:$innercode\n"

echo "*** create contract to alice"
viewcmd="$tools contract $alice $innercode"
echo $viewcmd
contract=`eval $viewcmd | sed -n '2p'`
echo -e "contract response:\n$contract\n"
...
echo $ouputid
#clause changePrice
#viewcmd="$tools unlockPriceChanger $ouputid $innercode $gold 123456 88 abc 00000000 99 $silver $alicerootpub $path1 $path2"
#clause redeem
viewcmd="$tools unlockPriceChanger $ouputid $alice $gold 123456 88 abc 33000000 $gold 77 $bob $control_program"
echo $viewcmd
result=`eval $viewcmd`
echo "$result"
