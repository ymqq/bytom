...
var RevealPreimage_body_bytes []byte

func init() {
	RevealPreimage_body_bytes, _ = hex.DecodeString("7caa87")
}
...
func PayToRevealPreimage(hash []byte) ([]byte, error) {
	...
	return compiler.Instantiate(RevealPreimage_body_bytes, _contractParams,  false, _contractArgs)
}
...
func ParsePayToRevealPreimage(prog []byte) ([][]byte, error) {
	...
	if !bytes.Equal(RevealPreimage_body_bytes, insts[1].Data) {
