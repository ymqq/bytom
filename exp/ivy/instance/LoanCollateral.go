...
var LoanCollateral_body_bytes []byte

func init() {
	LoanCollateral_body_bytes, _ = hex.DecodeString("557a641c00000000007251567ac1695100c3c251567ac163280000007bc59f690000c3c251577ac1")
}
...
// MINTIME LESSTHAN  [... borrower lender amountLoaned assetLoaned after(repaymentDue)]
...
func PayToLoanCollateral(assetLoaned bc.AssetID, amountLoaned uint64, repaymentDue time.Time, lender []byte, borrower []byte) ([]byte, error) {
	...
	return compiler.Instantiate(LoanCollateral_body_bytes, _contractParams,  false, _contractArgs)
}
...
func ParsePayToLoanCollateral(prog []byte) ([][]byte, error) {
	...
	if !bytes.Equal(LoanCollateral_body_bytes, insts[1].Data) {
