...
var TradeOffer_body_bytes []byte

func init() {
	TradeOffer_body_bytes, _ = hex.DecodeString("547a641300000000007251557ac1631a000000547a547aae7cac")
}
...
func PayToTradeOffer(assetRequested bc.AssetID, amountRequested uint64, seller []byte, cancelKey ed25519.PublicKey) ([]byte, error) {
	...
	return compiler.Instantiate(TradeOffer_body_bytes, _contractParams,  false, _contractArgs)
}
...
func ParsePayToTradeOffer(prog []byte) ([][]byte, error) {
	...
	if !bytes.Equal(TradeOffer_body_bytes, insts[1].Data) {
